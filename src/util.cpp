...
const char * const BITCOIN_CONF_FILENAME = "rapid.conf";
const char * const BITCOIN_PID_FILENAME = "rapidd.pid";
...
bool LogAcceptCategory(const char* category)
{
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL)
        {
            ...
            // "rapid" is a composite category enabling all Rapid-related debug output
            if(ptrCategory->count(string("rapid"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RapidCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RapidCore
    // Mac: ~/Library/Application Support/RapidCore
    // Unix: ~/.rapidcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RapidCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RapidCore";
#else
    // Unix
    return pathRet / ".rapidcore";
#endif
#endif
}
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty rapid.conf if it does not excist
    ...
    {
        // Don't overwrite existing settings so command line settings override rapid.conf
