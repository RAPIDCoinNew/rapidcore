...
{
LogPrintf("myfix net.cpp connectnode start\n");
...
LogPrintf("myfix net.cpp connectnode net active\n");
    if (pszDest == NULL) {
        ...
        if (IsLocal(addrConnect) && !fConnectToMasternode)
            return NULL;
LogPrintf("myfix net.cpp connectnode non masternode & islocal\n");
        LOCK(cs_vNodes);
...
LogPrintf("myfix net.cpp connectnode pnode=master\n");
            return pnode;
        }
    }
...
LogPrintf("myfix net.cpp connectnode connectsocketbyname ok\n");
...
LogPrintf("myfix net.cpp connectnode attempt ok\n");
        // Add node
...
LogPrintf("myfix net.cpp connetnode new cnode ok\n");
...
LogPrintf("myfix connectnode fail\n");
    return NULL;
}
...
void CNode::copyStats(CNodeStats &stats)
{
    ...
    // Raw ping time is in microseconds, but show it to user as whole seconds (Rapid users should be well used to small numbers with many decimal places by now :)
...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        std::string strDesc = "Rapid Core " + FormatFullVersion();
...
bool CConnman::BindListenPort(const CService &addrBind, std::string& strError, bool fWhitelisted)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Rapid Core is probably already running."), addrBind.ToString());
        else
