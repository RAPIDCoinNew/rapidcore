...
// ***TODO*** ScanHash is not yet used in Rapid
//
...
{
    printf("RapidMiner -- started\n");
    LogPrintf("RapidMiner -- started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("rapid-miner");
...
//    if(!coinbaseScript) printf("coinbaseScript error\n"); else printf("coinbaseScript OK\n");
//    if(coinbaseScript->reserveScript.empty())printf("reserve empty\n"); else printf("reserve non empty\n");
    try {
...
//if(fvNodesEmpty) printf("node Empty\n");
//if(IsInitialBlockDownload()) printf("non block download\n");
//if(!masternodeSync.IsSynced()) printf("non masternode sync\n");
            ...
            if (!pblocktemplate.get())
            {
                LogPrintf("RapidMiner -- Keypool ran out, please call keypoolrefill before restarting the mining thread\n");
                return;
            }
            ...
            LogPrintf("RapidMiner -- Running miner with %u transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
printf("RapidMiner -- terminated\n");
        LogPrintf("RapidMiner -- terminated\n");
        throw;
    }
    catch (const std::runtime_error &e)
    {
        LogPrintf("RapidMiner -- runtime error: %s\n", e.what());
        return;
    }
}
